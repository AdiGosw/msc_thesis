...
def run_all_conditions(n_runs=10):
    # Fixed sigma=5, vary N (N=2 already exists from DDPG.py)
    conditions = [
    ...
    for c in conditions:
        filename = os.path.join(script_dir, f"results_sigma{_fmt(c['sigma'])}_N{c['N']}.npy")
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*80}")
    print(f"{'SUMMARY — sigma=5, varying N':^80}")
    print(f"{'='*80}")
