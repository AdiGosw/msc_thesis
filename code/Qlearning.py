...
class Environment:
    def __init__(self, v_high: float = 4.0, v_low: float = 0.0, mu: float = 0.5, sigma: float = 5.0):
        self.v_high = v_high
        self.v_low = v_low
        self.mu = mu #Probablity of the asset value being high
    ...
    def step(self, prices: List[float]) -> Tuple[List[float], bool, dict]:
        ...
        Args:
            prices: List of prices from each agent
        Returns:
            profits: List of profits for each agent
            trade_occurred: Whether a trade happened
...
class QAgent:
    ...
    def check_convergence(self):
        ...
        if self.last_greedy_action is None:
            ...
            # Greedy action unchanged
            self.stable_action_episodes += 1
        else:
            # Greedy action changed, reset counter
            self.stable_action_episodes = 1
            self.last_greedy_action = current_greedy_action
    ...
    def finalize_learning(self):
        """Finalize the learning phase and prepare for exploitation"""
        if not self.converged:
            # Force finalization even if not converged
            self.final_q_values = self.q_values.copy()
    ...
    def step_learning(self) -> Tuple[int, float]:
        """Take one step during learning phase"""
    ...
    def step_exploitation(self) -> Tuple[int, float]:
        """Take one step during exploitation phase"""
    ...
    def learn(self, action: int, profit: float):
        """Update Q-values based on observed profit during learning phase"""
        if self.learning_complete:
        ...
        # Check convergence after updating
        self.check_convergence()
    
    def get_greedy_price(self) -> float:
        """Get current greedy price"""
        if self.learning_complete:
    ...
    def get_learned_strategy(self) -> Dict:
        """Get the final learned strategy for analysis"""
        if not self.learning_complete:
...
def run_full_simulation(K=10, n_jobs=14, sigma=5.0):
    """Run K experiments in parallel using joblib"""
    print(f"Running {K} experiments with {n_jobs} parallel jobs using joblib (sigma={sigma})...")
    start_time = time.time()

    # Use joblib for parallel processing
    results = Parallel(n_jobs=n_jobs, verbose=1)(
        delayed(run_single_experiment)(i, 100, sigma=sigma) for i in range(K)
    )
    
    total_time = time.time() - start_time
    print(f"Completed {K} experiments in {total_time:.1f}s ({total_time/60:.1f} minutes)")
    print(f"Average time per experiment: {total_time/K:.2f}s")
...
if __name__ == "__main__":
    ...
    print(f"Running {len(SIGMAS) * K} total experiments across sigmas {SIGMAS} with {N_JOBS} parallel jobs...")
    start_time = time.time()

    # Flat list of all (experiment_id, sigma) tasks
    ...
    # Group results by sigma and save
    for idx, sigma in enumerate(SIGMAS):
        ...
        filename = f'qlearning_sigma{sigma}_N2.npy'
